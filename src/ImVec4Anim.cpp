...
ImAnim::ImVec4Anim::ImVec4Anim(ImVec4* pVec4) : m_pVec4(pVec4)
{
...
const ImVec4& ImAnim::ImVec4Anim::getStartValue() const { return m_vecKeyValues[0].m_vValue; }
...
void ImAnim::ImVec4Anim::setKeyValueAt(double dStep, const ImVec4& vValue)
{
    ...
    for(size_t index = 0; index < m_vecKeyValues.size(); ++index)
    {
        ...
        if(dCurStep > dStep)
        {
            ...
            m_vecKeyValues.insert(m_vecKeyValues.begin() + index, KeyValue(dStep, vValue));
            break;
        }
    }
}
...
void ImAnim::ImVec4Anim::updateValueForProgress(double dProgress)
{
    ...
    if(dProgress > m_curEndKeyFrame.m_dStep)
    {
        ...
        for(size_t index = (m_nEndKeyFrameIndex + 1); index < m_vecKeyValues.size(); ++index)
        {
